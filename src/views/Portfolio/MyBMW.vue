...
<script>
	export default {
		data() {
			return {
				myQuantity: this.$store.state.myStocksQuantity.BMW,
				sellQuantity: 0
			};
		},
		methods: {
			sellStock() {
				if (this.sellQuantity <= this.myQuantity) {
					this.$store.state.myStocksQuantity.BMW -= parseInt(
						this.sellQuantity
					);
					this.myQuantity = this.$store.state.myStocksQuantity.BMW;
        ...
        computed:{
            price(){
                return this.$store.state.stockPrice.BMW;
            }
        }
	};
</script>
