...
<script>
	export default {
		data() {
			return {
				myQuantity: this.$store.state.myStocksQuantity.Twitter,
				sellQuantity: 0
			};
		},
		methods: {
			sellStock() {
				if (this.sellQuantity <= this.myQuantity) {
					this.$store.state.myStocksQuantity.Twitter -= parseInt(
						this.sellQuantity
					);
					this.myQuantity = this.$store.state.myStocksQuantity.Twitter;
        ...
        computed:{
            price(){
                return this.$store.state.stockPrice.Twitter;
            }
        }
	};
</script>
